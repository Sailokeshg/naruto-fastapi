...
async def get_episode_details_by_season_number(season_number: int, session: Session = Depends(get_yugabyte_db)):
...
async def get_episode_details_by_its_season_specific_episode_number(season_number: int, episode_number: int, session: Session = Depends(get_yugabyte_db)):

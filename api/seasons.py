...
async def get_episode_details_by_season_number(
        season_number: int, session: Session = Depends(get_postgres_db)):
...
async def get_episode_details_by_its_season_specific_episode_number(
        season_number: int, episode_number: int, session: Session = Depends(get_postgres_db)):
    '''Get episode details by season specific episode number'''
    episode_details = await get_complete_episode_details_by_season_number_and_episode_number(

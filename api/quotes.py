...
async def get_all_quotes(session: Session = Depends(get_yugabyte_db)):
...
async def get_characters_details(session: Session = Depends(get_yugabyte_db)) -> list:
...
async def get_quote_details_by_character_name(chatacter_name: Characters, session: Session = Depends(get_yugabyte_db)):

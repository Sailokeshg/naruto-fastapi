...
@router.get('/all')
async def get_all_episode_details(session: Session = Depends(get_yugabyte_db)):
    episode_details = await get_complete_episode_details(session)
    return episode_details

@router.get('/{episode_number}')
async def get_episode_details(episode_number: int, session: Session = Depends(get_yugabyte_db)):
    episode_details = await get_complete_episode_details_by_id(session, episode_number)
    return episode_details

@router.get('/types/{episode_type}')
async def get_episode_details_by_type(episode_type: EpisodeType, session: Session = Depends(get_yugabyte_db)):
...
async def get_episode_details_by_limit_and_offset(limit: int, offset: int, session: Session = Depends(get_yugabyte_db)):
